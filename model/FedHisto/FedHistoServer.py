...
from model.FedHisto.FedHistoClient import FedHistoAgent
import os


class FedHisto(ServerBase):
    def __init__(self, args, logger):
        super(FedHisto, self).__init__(args, logger)
        self.setup_clients()

    def setup_clients(self):
        for idx in self.n_clients:
            curr_client = FedHistoAgent(self.args, self.global_model, self.logger)
    ...
    def run(self, iter):
        ...
        for epoch in range(self.args.global_epochs):
            ...
            self.aggregate_parameter(local_weights, method='noise')
