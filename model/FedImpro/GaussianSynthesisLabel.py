...
class GaussianSynthesisLabel(FeatureSynthesis):
    ...
    def initial_model_params(self, feat, feat_length=None):
        self.feature_means = torch.rand(self.args.n_classes, feat.shape[1])
        if self.args.fed_split_std_mode == "update":
            self.feature_std = torch.ones([self.args.n_classes, feat.shape[1]])*self.args.fed_split_noise_std
        else:
