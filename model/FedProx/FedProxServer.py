...
from model.FedProx.FedProxClient import FedProxAgent
import os


class FedProx(ServerBase):
    def __init__(self, args, logger):
        super(FedProx, self).__init__(args, logger)
        self.setup_clients()

    def setup_clients(self):
        for idx in self.n_clients:
            curr_client = FedProxAgent(self.args, self.global_model, self.logger)
    ...
    def run(self, iter):
        ...
        for epoch in range(self.args.global_epochs):
            ...
            for idx in self.n_clients:
                self.clients[idx].update_global_model(self.global_model)
