...
from model.FedHE.FedHEClient import FedHEAgent
import os
...
class FedHE(ServerBase):
    def __init__(self, args, logger):
        super(FedHE, self).__init__(args, logger)
        self.setup_clients()
    ...
    def setup_clients(self):
        ...
        for idx in self.n_clients:
            curr_client = FedHEAgent(self.args, init_model, self.logger, MIL_pool)
    ...
    def run(self, iter):
        ...
        for epoch in range(self.args.global_epochs):
            local_weights, local_losses, local_lgts = [], [], {}
            self.logger.info(f'\n | Global Training Round : {epoch + 1} |\n')
            for idx in self.n_clients:
                w, agent_loss, lgts = self.clients[idx].local_train(idx, global_protos)
                local_weights.append(deepcopy(w))
                local_losses.append(deepcopy(agent_loss))
                agg_lgts = agg_func(lgts)
                local_lgts[idx] = agg_lgts
            ...
            # update global weights
            global_protos = proto_aggregation(local_lgts)
        ...
        return best_accuracy, train_acc_wt, list_acc