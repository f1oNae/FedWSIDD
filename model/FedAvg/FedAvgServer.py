...
from model.FedAvg.FedAvgClient import FedAvgAgent
import os


class FedAvg(ServerBase):
    def __init__(self, args, logger):
        super(FedAvg, self).__init__(args, logger)
        self.setup_clients()

    def setup_clients(self):
        for idx in self.n_clients:
            curr_client = FedAvgAgent(self.args, self.global_model, self.logger)
    ...
    def run(self, iter):
        ...
        for epoch in range(self.args.global_epochs):
            ...
            for idx in self.n_clients:
                ...
                print(f'FedAvg Comm Costs: {n_elements}')
