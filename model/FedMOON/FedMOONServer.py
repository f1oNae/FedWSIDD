...
from model.FedMOON.FedMOONClient import FedMOONAgent
import os


class FedMOON(ServerBase):
    def __init__(self, args, logger):
        super(FedMOON, self).__init__(args, logger)
        self.setup_clients()

    def setup_clients(self):
        for idx in self.n_clients:
            curr_client = FedMOONAgent(self.args, self.global_model, self.logger)
    ...
    def run(self, iter):
        ...
        old_models_pool = []
        local_models = {}
        for i in self.n_clients:
            local_models[i] = deepcopy(self.clients[i].local_model)
            local_models[i].to(self.device)
        ...
        for epoch in range(self.args.global_epochs):
            ...
            for idx in self.n_clients:
                ...
                local_models[idx] = deepcopy(self.clients[idx].local_model)

            # update global weights
            self.aggregate_parameter(local_weights, method='weighted')
            for idx in self.n_clients:
